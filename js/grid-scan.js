/**
 * GridScan (vanilla port of React Bits GridScan).
 */
(function registerBeniniGridScan(global) {
  const MAX_SCANS = 8;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  // Centro transparente (fade radial) para revelar o video no background.
  // p esta em space normalizado; 0.0 = centro.
  float centerR = 0.22;
  float centerFeather = 0.26;
  float centerFade = smoothstep(centerR, centerR + centerFeather, length(p));
  alpha *= centerFade;
  color *= mix(0.65, 1.0, centerFade);
...
`;
  ...
  function scanDirectionValue(scanDirection) {
    if (scanDirection === 'backward') return 1;
    if (scanDirection === 'pingpong') return 2;
    return 0;
  }

  function lineStyleValue(lineStyle) {
    if (lineStyle === 'dashed') return 1;
    if (lineStyle === 'dotted') return 2;
    return 0;
  }

  function initBeniniGridScan(section, container, options) {
    if (!section || !container || typeof THREE === 'undefined') return null;

    const opts = { ...DEFAULT_OPTIONS, ...options };
    const sensitivity = THREE.MathUtils.clamp(opts.sensitivity, 0, 1);
    const hoverIntensity = THREE.MathUtils.clamp(
      typeof opts.hoverIntensity === 'number' ? opts.hoverIntensity : 1,
      0,
      1
    );
    ...
    const uniforms = {
      ...
      uLineStyle: { value: lineStyleValue(opts.lineStyle) },
      ...
      uScanDirection: { value: scanDirectionValue(opts.scanDirection) },
    ...
    const onMove = (e) => {
      ...
      lookTarget.set(nx * hoverIntensity, ny * hoverIntensity);
    };
    ...
    const tick = () => {
      ...
      const tiltSm = smoothDampFloat(tiltCurrent.value, tiltTarget.value, tiltVel, smoothTime, maxSpeed, dt);
      tiltCurrent.value = tiltSm.value;

      const yawSm = smoothDampFloat(yawCurrent.value, yawTarget.value, yawVel, smoothTime, maxSpeed, dt);
      yawCurrent.value = yawSm.value;
      ...
      uniforms.uTilt.value = tiltCurrent.value * tiltScale;
      uniforms.uYaw.value = THREE.MathUtils.clamp(yawCurrent.value * yawScale, -0.6, 0.6);
    ...
    return function disposeBeniniGridScan() {
  ...
  global.initBeniniGridScan = initBeniniGridScan;
})(window);
